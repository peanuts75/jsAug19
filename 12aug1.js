...
var me = { // this is how you create an object; simply add attributes
...
console.log(gp1) // if I were to use these pieces, it would end up starting with these two lines
console.log(gp3, sg[0][0],gp4, sg[0][1], gp4, sg[0][2], gp3) // this is effectively just a couple of borders, the first row of grid pieces and a few joining parts
...
sg[0][1] = "o"
sg[0][2] = "o"
function checkLine(ch1, ch2, ch3){
    if (ch1 == ch2 && ch1 == ch3 && ch1 != " ") {
        return ch1}
    else{return null}}
winner = "no one"
function checkwinner(chxyd, chno){
    if(chxyd == "x"){
        checkLine(sg[chno][0], sg[chno][1], sg[chno][2])
    }
    else if(chxyd == "y")
        checkLine(sg[0][chno], sg[1][chno], sg[2][chno])
    }
    else if(chno == )
}

console.log(checkLine(sg[0][0], sg[0][1], sg[0][2]))